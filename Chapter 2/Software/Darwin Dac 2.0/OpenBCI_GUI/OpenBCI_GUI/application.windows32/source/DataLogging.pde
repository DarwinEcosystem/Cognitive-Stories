...
public class OutputFile_rawtxt {
  ...
  private void writeValues(int[] values, float scale_fac) {
    ...
    for (int Ival = 0; Ival < nVal; Ival++) {
      ...
      output.print(String.format(Locale.US, "%f", abs(scale_fac * float(values[Ival]))   ));
      
      // nn file output
      
      output_nn.print(String.format(Locale.US, "%f", abs(scale_fac /10000  * float(values[Ival])) /500   ));
      if((Ival +1 < nVal)) {
        output.print(", ");
        output_nn.print(", ");
      }
  ...
  private void writeAccValues(int[] values, float scale_fac) {
    ...
    for (int Ival = 0; Ival < nVal; Ival++) {
      ...
      output_nn.print(", ");
      output_nn.print(String.format(Locale.US, "%.3f", scale_fac * float(values[Ival])));
    }
  }
