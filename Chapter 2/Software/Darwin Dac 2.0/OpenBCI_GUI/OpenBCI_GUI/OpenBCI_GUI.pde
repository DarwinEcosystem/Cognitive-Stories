...
void initSystem() {
  ...
  if (eegDataSource == DATASOURCE_PLAYBACKFILE) {
    ...
    println("Darwin_DAC_GUI: initSystem: loading complete.  " + playbackData_table.getRowCount() + " rows of data, which is " + round(float(playbackData_table.getRowCount())/getSampleRateSafe()) + " seconds of EEG data");
...
void systemDraw() { //for drawing to the screen
  ...
  if (systemMode >= SYSTEMMODE_POSTINIT && !initSystemThreadLock) {
    ...
    surface.setTitle(int(frameRate) + " fps — Darwin Ecosystem DAC GUI");
  }
...
void introAnimation() {
  ...
  if (millis() >= t1) {
    ...
    image(cog, width/2, height/2, width/1.75, width/6);
    textFont(p3, 16);
    textLeading(24);
