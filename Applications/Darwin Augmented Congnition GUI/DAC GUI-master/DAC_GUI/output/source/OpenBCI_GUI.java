...
public void initSystem(){
  ...
  switch (eegDataSource) {
    ...
    case DATASOURCE_PLAYBACKFILE:
      ...
      println("DAC_GUI: loading playback data from " + playbackData_fname);
      try {
      ...
      println("DAC_GUI: loading complete.  " + playbackData_table.getRowCount() + " rows of data, which is " + round(PApplet.parseFloat(playbackData_table.getRowCount())/openBCI.fs_Hz) + " seconds of EEG data");
  ...
  if(systemMode == 10){
    if (isRunning) {
      ...
      if (pointCounter >= nPointsPerUpdate) {
        ...
        //try to detect the desired signals, do it in frequency space...for DAC_GUI_Simpler
...
public void systemDraw(){ //for drawing to the screen
  ...
  if(systemMode == 10){
    ...
    if ((redrawScreenNow) || (drawLoop_counter >= drawLoopCounter_thresh)) {
      //if (drawLoop_counter >= drawLoopCounter_thresh) println("DAC_GUI: redrawing based on loop counter...");
...
public int getDataIfAvailable(int pointCounter) {
  ...
  if ( (eegDataSource == DATASOURCE_NORMAL) || (eegDataSource == DATASOURCE_NORMAL_W_AUX) ) {
    ...
    if (current_millis >= nextPlayback_millis) {
      ...
      //if (eegDataSource==DATASOURCE_PLAYBACKFILE) println("DAC_GUI: getDataIfAvailable: currentTableRowIndex = " + currentTableRowIndex);
      //println("DAC_GUI: getDataIfAvailable: pointCounter = " + pointCounter);
  ...
  if(systemMode >= 10){
    ...
    //      }
      ...
      if (gui.isMouseOnFFT(mouseX,mouseY)) {
        ...
        println("DAC_GUI: FFT data point: " + String.format("%4.2f",dataPoint.x) + " " + dataPoint.x_units + ", " + String.format("%4.2f",dataPoint.y) + " " + dataPoint.y_units);
...
public void stopButtonWasPressed() {
  ...
  if (isRunning) {
    println("DAC_GUI: stopButton was pressed...stopping data transfer...");
    stopRunning();
  } 
  else { //not running
    println("DAC_GUI: startButton was pressed...starting data transfer...");
    startRunning();
    nextPlayback_millis = millis();  //used for synthesizeData and readFromFile.  This restarts the clock that keeps the playback at the right pace.
  }
}
...
public void updateButtons(){
  ...
  if (isRunning) {
    //println("DAC_GUI: stopButtonWasPressed (a): changing string to " + Gui_Manager.stopButton_pressToStop_txt);
    gui.stopButton.setString(Gui_Manager.stopButton_pressToStop_txt); 
    gui.stopButton.setColorNotPressed(color(224, 56, 45));
  } 
  else {
    //println("DAC_GUI: stopButtonWasPressed (a): changing string to " + Gui_Manager.stopButton_pressToStart_txt);
...
public int getPlaybackDataFromTable(Table datatable, int currentTableRowIndex, float scale_fac_uVolts_per_count, DataPacket_ADS1299 curDataPacket) {
  ...
  if (currentTableRowIndex >= datatable.getRowCount()) {
    //end of file
    println("DAC_GUI: hit the end of the playback data file.  starting over...");
...
public void activateChannel(int Ichan) {
  println("DAC_GUI: activating channel " + (Ichan+1));
  if(eegDataSource == DATASOURCE_NORMAL || eegDataSource == DATASOURCE_NORMAL_W_AUX){
    if (serial_openBCI != null){
      verbosePrint("**");
...
public void deactivateChannel(int Ichan) {
  println("DAC_GUI: deactivating channel " + (Ichan+1));
  if(eegDataSource == DATASOURCE_NORMAL || eegDataSource == DATASOURCE_NORMAL_W_AUX){
    if (serial_openBCI != null) {
      verbosePrint("***");
...
//   println("DAC_GUI: toggleChannelImpedanceState: Ichan " + Ichan + ", code_P_N_Both " + code_P_N_Both);
...
public void openNewLogFile(String _fileName) {
  ...
  if (fileoutput != null) {
    println("DAC_GUI: closing log file");
    closeLogFile();
  }
...
//          println("DAC_GUI: executing shutdown code...");
//          try {
//            stopRunning();
//            if (openBCI != null) {
...
//    updateMaxDisplayFreq();
  ...
  public void setupFFTPlot(Graph2D g, int win_x, int win_y, float[] axis_relPos,PlotFontInfo fontInfo) {
    ...
    g.setYAxisMinorTicks(0);
    g.setYAxisLabelAccuracy(0);
    //g.setYAxisLabel("EEG Amplitude (uV/sqrt(Hz))");  // Some people prefer this...but you'll have to change the normalization in DAC_GUI\processNewData()
    g.setYAxisLabel("EEG Amplitude (uV per bin)");  // CHIP 2014-10-24...currently, this matches the normalization in DAC_GUI\processNewData()
  ...
  public void initializeMontageTraces(float[] dataBuffX, float [][] dataBuffY) {
    ...
    //montageTrace.setYScaleFac(1.0f); //for DAC_GUI_Simpler
...
public void keyPressed() {
  ...
  //println("DAC_GUI: keyPressed: key = " + key + ", int(key) = " + int(key) + ", keyCode = " + keyCode);
...
public void parseKey(char val) {
  ...
  switch (val) {
    ...
    case 'm':
     ...
     println("DAC_GUI: 'm' was pressed...taking screenshot:" + picfname);
     saveFrame(picfname);    // take a shot of that!
     break;

    default:
     println("DAC_GUI: '" + key + "' Pressed...sending to OpenBCI...");
...
public void parseKeycode(int val) { 
  ...
  switch (val) {
    case 8:
      println("DAC_GUI: parseKeycode(" + val + "): received BACKSPACE keypress.  Ignoring...");
      break;   
    case 9:
      println("DAC_GUI: parseKeycode(" + val + "): received TAB keypress.  Ignoring...");
      //gui.showImpedanceButtons = !gui.showImpedanceButtons;
      // gui.incrementGUIpage(); //deprecated with new channel controller
      break;    
    case 10:
      println("DAC_GUI: parseKeycode(" + val + "): received ENTER keypress.  Ignoring...");
      break;
    case 16:
      println("DAC_GUI: parseKeycode(" + val + "): received SHIFT keypress.  Ignoring...");
      break;
    case 17:
      //println("DAC_GUI: parseKeycode(" + val + "): received CTRL keypress.  Ignoring...");
      break;
    case 18:
      println("DAC_GUI: parseKeycode(" + val + "): received ALT keypress.  Ignoring...");
      break;
    case 20:
      println("DAC_GUI: parseKeycode(" + val + "): received CAPS LOCK keypress.  Ignoring...");
      break;
    case 27:
      println("DAC_GUI: parseKeycode(" + val + "): received ESC keypress.  Stopping OpenBCI...");
      stopRunning();
      break; 
    case 33:
      println("DAC_GUI: parseKeycode(" + val + "): received PAGE UP keypress.  Ignoring...");
      break;    
    case 34:
      println("DAC_GUI: parseKeycode(" + val + "): received PAGE DOWN keypress.  Ignoring...");
      break;
    case 35:
      println("DAC_GUI: parseKeycode(" + val + "): received END keypress.  Ignoring...");
      break; 
    case 36:
      println("DAC_GUI: parseKeycode(" + val + "): received HOME keypress.  Ignoring...");
      break; 
    case 37:
      println("DAC_GUI: parseKeycode(" + val + "): received LEFT ARROW keypress.  Ignoring...");
      break;  
    case 38:
      println("DAC_GUI: parseKeycode(" + val + "): received UP ARROW keypress.  Ignoring...");
      break;  
    case 39:
      println("DAC_GUI: parseKeycode(" + val + "): received RIGHT ARROW keypress.  Ignoring...");
      break;  
    case 40:
      println("DAC_GUI: parseKeycode(" + val + "): received DOWN ARROW keypress.  Ignoring...");
      break;
    case 112:
      println("DAC_GUI: parseKeycode(" + val + "): received F1 keypress.  Ignoring...");
      break;
    case 113:
      println("DAC_GUI: parseKeycode(" + val + "): received F2 keypress.  Ignoring...");
      break;  
    case 114:
      println("DAC_GUI: parseKeycode(" + val + "): received F3 keypress.  Ignoring...");
      break;  
    case 115:
      println("DAC_GUI: parseKeycode(" + val + "): received F4 keypress.  Ignoring...");
      break;  
    case 116:
      println("DAC_GUI: parseKeycode(" + val + "): received F5 keypress.  Ignoring...");
      break;  
    case 117:
      println("DAC_GUI: parseKeycode(" + val + "): received F6 keypress.  Ignoring...");
      break;  
    case 118:
      println("DAC_GUI: parseKeycode(" + val + "): received F7 keypress.  Ignoring...");
      break;  
    case 119:
      println("DAC_GUI: parseKeycode(" + val + "): received F8 keypress.  Ignoring...");
      break;  
    case 120:
      println("DAC_GUI: parseKeycode(" + val + "): received F9 keypress.  Ignoring...");
      break;  
    case 121:
      println("DAC_GUI: parseKeycode(" + val + "): received F10 keypress.  Ignoring...");
      break;  
    case 122:
      println("DAC_GUI: parseKeycode(" + val + "): received F11 keypress.  Ignoring...");
      break;  
    case 123:
      println("DAC_GUI: parseKeycode(" + val + "): received F12 keypress.  Ignoring...");
      break;     
    case 127:
      println("DAC_GUI: parseKeycode(" + val + "): received DELETE keypress.  Ignoring...");
      break;
    case 155:
      println("DAC_GUI: parseKeycode(" + val + "): received INSERT keypress.  Ignoring...");
      break; 
    default:
      println("DAC_GUI: parseKeycode(" + val + "): value is not known.  Ignoring...");
      break;
  }
}
//////////////////////////////////////////////////////////////////////////
