...
void initSystem() {
  ...
  case DATASOURCE_PLAYBACKFILE:
    ...
    println(appName + ": initSystem: loading complete.  " + playbackData_table.getRowCount() + " rows of data, which is " + round(float(playbackData_table.getRowCount())/get_fs_Hz_safe()) + " seconds of EEG data");
...
void systemDraw() { //for drawing to the screen
  ...
  if (systemMode >= SYSTEMMODE_POSTINIT) {
    ...
    surface.setTitle(int(frameRate) + " fps — Augmented Cognition GUI");
  }
